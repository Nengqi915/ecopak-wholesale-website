...
import { coffeeBeansBagsProducts } from './coffeeBeansBagsData';

export const CoffeeBeansBags = ({ setPage }: { setPage: (p: string) => void }) => (
  <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-32">
    <div className="mb-20 text-center">
      <span className="text-[#82C864] font-black text-sm tracking-[0.3em] uppercase mb-4 block">Premium Packaging</span>
      <h1 className="text-5xl lg:text-7xl font-black mb-8 text-[#1A1A1A]">Coffee Beans <span className="text-[#FF9F1C]">Bags</span></h1>
      <p className="text-xl text-gray-500 max-w-2xl mx-auto font-medium leading-relaxed">
        Elevate your roastery's brand with our premium, high-barrier coffee bags designed for maximum freshness and visual impact.
      </p>
    </div>

    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
      {coffeeBeansBagsProducts.map((product, index) => (
        <motion.div
          key={product.id}
          ...
          onClick={() => {
            setPage(`coffee-beans-bags/${product.id}`);
            window.scrollTo({ top: 0, behavior: 'smooth' });
          }}
          className="group cursor-pointer"
        >
          <div className="bg-gray-50 rounded-[2.5rem] p-8 aspect-square mb-6 flex justify-center items-center relative overflow-hidden shadow-lg border border-gray-100 group-hover:shadow-2xl group-hover:shadow-[#82C864]/20 transition-all duration-500">
            <img 
              src={product.image} 
              alt={product.name}
              className="w-[80%] object-contain group-hover:scale-110 transition-transform duration-700" 
            />
          ...
          <h3 className="text-2xl font-black mb-3 text-[#1A1A1A] group-hover:text-[#FF9F1C] transition-colors">{product.name}</h3>
          <p className="text-gray-500 font-medium line-clamp-2">{product.description}</p>
        </motion.div>
      ))}
    </div>
  </div>
...
export const CoffeeBeansBagsDetail = ({ pageId, setPage, openInquiry }: { pageId: string; setPage: (p: string) => void; openInquiry: () => void }) => {
  const item = coffeeBeansBagsProducts.find(p => p.id === pageId);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-32">
      <button 
        onClick={() => setPage('coffee-beans-bags')}
      ...
      >
        <ChevronRight className="w-4 h-4 rotate-180" /> Back to Coffee Beans Bags
      </button>
