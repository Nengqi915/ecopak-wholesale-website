...
import { oneStopProducts } from './oneStopData';

export const OneStop = ({ setPage }: { setPage: (p: string) => void }) => (
  <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-32">
    <div className="mb-20 text-center">
      <span className="text-[#82C864] font-black text-sm tracking-[0.3em] uppercase mb-4 block">Complete Solutions</span>
      <h1 className="text-5xl lg:text-7xl font-black mb-8 text-[#1A1A1A]">One-Stop <span className="text-[#FF9F1C]">Packaging</span></h1>
      <p className="text-xl text-gray-500 max-w-2xl mx-auto font-medium leading-relaxed">
        Comprehensive packaging suites designed to cover all your cafe's needs in one unified brand style.
      </p>
    </div>

    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
      {oneStopProducts.map((product, index) => (
        <motion.div
          key={product.id}
          ...
          onClick={() => {
            setPage(`one-stop-packaging/${product.id}`);
...
export const OneStopDetail = ({ pageId, setPage, openInquiry }: { pageId: string; setPage: (p: string) => void; openInquiry: () => void }) => {
  const item = oneStopProducts.find(p => p.id === pageId);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-32">
      <button 
        onClick={() => setPage('one-stop-packaging')}
      ...
      >
        <ChevronRight className="w-4 h-4 rotate-180" /> Back to One-Stop Packaging
      </button>
